...
def make_data(n: int, n_descr: int = 500) -> pl.DataFrame:
    ...
    rows = {
        ...
        "ncm": [
            random.choice([None, f"{random.randint(1000,9999)}"]) for _ in range(n)
        ],
        "cest": [
            random.choice([None, f"{random.randint(1000,9999)}"]) for _ in range(n)
        ],
        "gtin": [
            random.choice([None, f"{random.randint(1000000000000,9999999999999)}"])
            for _ in range(n)
        ],
        "co_sefin_item": [
            random.choice([None, f"CS{random.randint(1,9)}"]) for _ in range(n)
        ],
    }
...
def bench(n: int):
    ...
    grouped = (
        ...
        .with_columns(
            [
                pl.when(pl.col("ncm").is_not_null())
                .then(1)
                .otherwise(0)
                .alias("__has_ncm"),
                pl.when(pl.col("cest").is_not_null())
                .then(1)
                .otherwise(0)
                .alias("__has_cest"),
                pl.when(pl.col("gtin").is_not_null())
                .then(1)
                .otherwise(0)
                .alias("__has_gtin"),
            ]
