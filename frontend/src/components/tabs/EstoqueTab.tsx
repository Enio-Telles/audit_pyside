...
function EstoqueSubTab({ cnpj, sub }: { cnpj: string; sub: SubTab }) {
  ...
  const filteredRows = useMemo(() => {
    return (data?.rows ?? []).filter((r) => {
      if (filterIdAgrupado && String(r['id_agrupado'] ?? '') !== filterIdAgrupado) return false;
      if (search) {
    ...
    <div className="flex flex-col h-full">
      <div className="px-3 py-2 border-b border-slate-700">
        ...
        <div className="flex gap-2 items-center flex-wrap">
          ...
          {hasFilters && (
            <button
              className={btnCls + ' bg-slate-700 hover:bg-slate-600 text-slate-400'}
        ...
        <div className="flex items-center gap-3 mt-1">
          ...
          {hasFilters && (
            <span className="text-xs text-amber-400">
              Filtros ativos: {[filterIdAgrupado && 'id_agrupado', search && 'descricao'].filter(Boolean).join(', ')}
            </span>
          )}
        </div>
      </div>
