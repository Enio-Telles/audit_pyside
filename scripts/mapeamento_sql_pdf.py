        ...
        with open(sql_file, "r", encoding="utf-8") as f:
            for line in f:
                if "--" in line:
                    ...
                    match = re.search(
                        r"(?:[A-Za-z0-9_]+\.)?([A-Za-z0-9_]+)\s*(?:AS\s+[A-Za-z0-9_]+)?\s*,?\s*--\s*(.*)",
                        line,
                        re.IGNORECASE,
                    )
                    if match:
...
def get_all_tables_from_schemas(schemas):
    ...
    try:
        ...
        log_info(
            f"Aviso - Não foi possível ler listagem de objetos para os schemas {schemas}: {e}"
        )
...
def get_columns_from_db(schema_name, table_name):
    ...
    try:
        ...
        log_info(
            f"Aviso - Não foi possível ler colunas para {schema_name}.{table_name}: {e}"
        )
...
def main():
    ...
    if not tabelas_db:
        log_info(
            "Nenhuma tabela retornada do Banco de Dados para os schemas identificados."
        )
        return

    log_info(
        f"Iniciando processamento de {len(tabelas_db)} tabelas extraídas do Banco de Dados..."
    )
    ...
    log_info(
        f"Processo concluído: {processados} novas tabelas mapeadas, {pulados} já existiam."
    )
