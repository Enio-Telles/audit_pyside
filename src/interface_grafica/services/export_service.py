...
class ExportService:
    ...
    def export_excel(
        ...
        gerado = exportar_excel_adaptado(
            df=df,
            nome_base=target.stem,
            diretorio_saida=target.parent,
            nome_aba=sheet_name[:31] if sheet_name else "Dados",
        )
        if gerado is not None and gerado != target:
            gerado.replace(target)
        return target
