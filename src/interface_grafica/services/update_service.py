    ...
    def run(self):
        try:
            ...
            if self.is_newer(tag_name, __version__):
                ...
                if zip_url:
                    self.finished.emit(ReleaseInfo(
                        tag_name=tag_name,
                        body=data.get("body", ""),
                        zip_url=zip_url,
...
class UpdateService(QObject):
    ...
    def _on_checksums_downloaded(self, reply, release_info: ReleaseInfo):
        if reply.error() != QNetworkReply.NoError:
            logger.warning(f"Failed to download checksums: {reply.errorString()}")
            # Continue without hash verification if checksums.txt is missing
            self._download_zip(release_info.zip_url)
            return
