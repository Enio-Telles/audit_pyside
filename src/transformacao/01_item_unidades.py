...
def _ler_nfe_ou_nfce(
    ...
    gtin_expr = (
        ...
        if "prod_ceantrib" in lf_selected.columns or "prod_cean" in lf_selected.columns
    ...
    return lf_selected.with_columns(
        [
            (
                ...
                if "prod_cprod" in lf_selected.columns
                else pl.lit(None, pl.String).alias("codigo")
            ),
            (
                _normalizar_descricao_completa("prod_xprod").alias("descricao")
                if "prod_xprod" in lf_selected.columns
            ...
            (
                ...
                if "prod_ncm" in lf_selected.columns
                else pl.lit(None, pl.String).alias("ncm")
            ),
            (
                _normalizar_texto("prod_cest").alias("cest")
                if "prod_cest" in lf_selected.columns
                else pl.lit(None, pl.String).alias("cest")
            ),
            gtin_expr.alias("gtin"),
            (
                _normalizar_texto("prod_ucom").alias("unid")
                if "prod_ucom" in lf_selected.columns
                else pl.lit(None, pl.String).alias("unid")
            ),
            pl.lit(0.0).alias("compras"),
            pl.lit(0.0).alias("qtd_compras"),
...
def item_unidades(cnpj: str, pasta_cnpj: Path | None = None) -> bool:
    ...
    lf_grouped = (
        lf_total.group_by(chaves_agrupamento)
        .agg(
            [
