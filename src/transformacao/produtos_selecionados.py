...
def gerar_produtos_selecionados_dataframe(
    mensal: pl.DataFrame, anual: pl.DataFrame, mov_estoque: pl.DataFrame
) -> pl.DataFrame:
    ...
    if not mov_estoque.is_empty():
        col_id = (
            "id_agregado"
            if "id_agregado" in mov_estoque.columns
            else ("id_agrupado" if "id_agrupado" in mov_estoque.columns else None)
        )
        if col_id:
            bases_id.append(
                mov_estoque.select(
    ...
    if not bases_id:
        return pl.DataFrame(
            schema={
                ...
                "em_mensal": pl.Boolean,
                "em_anual": pl.Boolean,
                "em_estoque": pl.Boolean,
            }
        )

    master = (
        pl.concat(bases_id, how="vertical_relaxed")
    ...
    # Flags de presença
    ids_mensal = (
        mensal.get_column("id_agregado").unique()
        if not mensal.is_empty()
        else pl.Series(dtype=pl.Utf8)
    )
    ids_anual = (
        anual.get_column("id_agregado").unique()
        if not anual.is_empty()
        else pl.Series(dtype=pl.Utf8)
    )
    ...
    ids_mov = (
        mov_estoque.get_column(col_id_mov).unique()
        if (not mov_estoque.is_empty() and col_id_mov)
        else pl.Series(dtype=pl.Utf8)
    )
