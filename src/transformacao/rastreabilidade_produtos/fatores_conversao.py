...
def _salvar_log_sem_compra(
    df_sem_compra: pl.DataFrame, pasta_analises: Path, cnpj: str
) -> None:
    salvar_para_parquet(
...
def _carregar_agrupamento_canonico(pasta_analises: Path, cnpj: str) -> pl.DataFrame:
    ...
    def _selecionar_primeira_lista_nao_vazia(
        df_grouped: pl.DataFrame, source_col: str, target_col: str
    ) -> pl.DataFrame:
        ...
        if source_col in df_grouped.columns:
            ...
            df_grouped = df_grouped.with_columns(
                pl.Series(nova_col).alias(target_col)
            ).drop(cleaned_col)
        else:
            df_grouped = df_grouped.with_columns(
                pl.lit([]).cast(pl.List(pl.Utf8)).alias(target_col)
            )
        return df_grouped

    # Aplicar helper para as duas colunas de listas (evita duplicacao de codigo)
    df_grouped = _selecionar_primeira_lista_nao_vazia(
        df_grouped, "__listas_descricoes", "lista_descricoes"
    )
    df_grouped = _selecionar_primeira_lista_nao_vazia(
        df_grouped, "__listas_desc_compl", "lista_desc_compl"
    )

    return df_grouped.drop(["__listas_descricoes", "__listas_desc_compl"])


def _construir_mapa_descricoes_canonicas(
    ...
    return (
        ...
        .with_columns(
            [
                ...
                .str.strip_chars()
                .alias("descricao_texto"),
                normalizar_descricao_expr("descricao_texto").alias(
                    "descricao_normalizada_match"
                ),
            ]
        )
...
def _salvar_log_reconciliacao_overrides(
    ...
    with open(
        pasta_analises / f"log_reconciliacao_overrides_fatores_{cnpj}.json",
        "w",
        encoding="utf-8",
    ) as f:
        json.dump(resumo, f, ensure_ascii=False, indent=2)
...
def _reconciliar_fatores_existentes_com_agrupamento_atual(
    ...
    tem_flags_manuais = {"fator_manual", "unid_ref_manual"}.intersection(
    ...
    df_avaliacao = (
        ...
        .with_columns(
            [
                (pl.col("fator_manual") | pl.col("unid_ref_manual")).alias(
                    "__eh_manual__"
                ),
                normalizar_descricao_expr("descr_padrao").alias(
                    "__descr_padrao_norm__"
                ),
                normalizar_descricao_expr("descr_padrao_canonico").alias(
                    "__descr_padrao_canonico_norm__"
                ),
            ]
        )
        .with_columns(
    ...
    df_descartados = df_manuais_incoerentes.filter(
        pl.col("id_agrupado_destino").is_null()
    )
        ...
        [
            df_remapeados.select(
                [
                    ...
                    pl.col("unid_ref"),
                    pl.lit(
                        "Override manual realocado para o agrupamento canonico atual."
                    ).alias("motivo"),
                ]
            ),
            df_descartados.select(
                [
                    ...
                    pl.col("unid_ref"),
                    pl.lit(
                        "Override manual descartado por nao haver correspondencia unica no agrupamento atual."
                    ).alias("motivo"),
                ]
            ),
        ],
...
def calcular_fatores_conversao(cnpj: str, pasta_cnpj: Path | None = None) -> bool:
    ...
    df_fatores_existente = (
        pl.read_parquet(arq_fatores_existente)
        if arq_fatores_existente.exists()
        else pl.DataFrame()
    )
    ...
    if df_link.is_empty():
        rprint(
            "[yellow]Aviso: sem vinculacao entre item_unidades e produtos_final.[/yellow]"
        )
        return salvar_para_parquet(
            pl.DataFrame(), pasta_analises, f"fatores_conversao_{cnpj}.parquet"
        )
    ...
    unid_ref_manual = (
        ...
        .with_columns(
            pl.col("unid_ref_sugerida").cast(pl.String, strict=False).str.strip_chars()
        )
        .filter(
            pl.col("unid_ref_sugerida").is_not_null()
            & (pl.col("unid_ref_sugerida") != "")
        )
        .group_by("id_agrupado")
    ...
    if not df_fatores_existente.is_empty():
        ...
        if colunas_manual:
            df_manuais_antigos = df_fatores_existente.with_columns(
                colunas_manual
            ).filter(
                ...
                if not df_orfaos.is_empty():
                    ...
                    if "id_produtos" in df_orfaos.columns:
                        df_orfaos = df_orfaos.with_columns(
                            pl.coalesce(
                                [pl.col("id_produtos"), pl.col("id_agrupado")]
                            ).alias("id_produtos")
                        )

                    df_orfaos = df_orfaos.select(df_fatores.columns)
                    df_fatores = pl.concat(
                        [df_fatores, df_orfaos], how="vertical_relaxed"
                    )
                    df_fatores = df_fatores.unique(
                        subset=["id_agrupado", "unid"], keep="first"
                    ).sort(["id_agrupado", "unid"])
                    rprint(
                        f"[green]Preservados {df_orfaos.height} fatores manuais orfaos.[/green]"
                    )

    return salvar_para_parquet(
        df_fatores, pasta_analises, f"fatores_conversao_{cnpj}.parquet"
    )
