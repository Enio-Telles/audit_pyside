...
def _salvar_auditoria_fora_escopo(
    ...
    ok = salvar_para_parquet(df_fora_escopo, pasta_analises, nome_arquivo)
    if ok:
        rprint(
...
def _candidatos_cfop_bi() -> list[Path]:
    return [
        CFOP_BI_PATH,
        DADOS_DIR / "referencias" / "cfop" / "cfop_bi.parquet",
...
def _processar_fonte_em_batches_anuais(
    ...
    for ano in anos:
        ...
        df_ano = df_ano.with_columns(_normalizar_descricao_expr(col_desc))
    ...
    if batches_fora_escopo:
        ...
        if not _salvar_auditoria_fora_escopo(
            df_fora_escopo_total, pasta_analises, fonte, cnpj
        ):
            return False
...
def gerar_fontes_produtos(cnpj: str, pasta_cnpj: Path | None = None) -> bool:
    ...
    for fonte in fontes:
        ...
        df_src = df_src.with_columns(_normalizar_descricao_expr(col_desc))
