...
def aux_classif_merc(
    ...
    if not all(p.exists() for p in arquivos_obrigatorios):
        raise FileNotFoundError(
            "Um ou mais arquivos de referência sitafe não foram encontrados na pasta referencias/CO_SEFIN/"
        )

    rprint("[blue]Carregando bases de referência NCM/CEST SEFIN...[/blue]")
    df_cest_ncm = (
        pl.read_parquet(path_cest_ncm)
        .select(
    ...
    df = df.join(df_cest, left_on=["_temp_cest"], right_on=["ref_cest"], how="left")

    # 3. Cruzamento apenas NCM (Fallback)
    df = df.join(df_ncm, left_on=["_temp_ncm"], right_on=["ref_ncm"], how="left")

    # 4. Resolve o fallback usando coalesce e garante tipo string
    df = df.with_columns(
        pl.coalesce(["sefin_cest_ncm", "sefin_cest", "sefin_ncm"])
        .cast(pl.String)
    ...
    if col_dhemi and col_dhsaient:
            ...
            .filter(
                ...
                | (
                    ...
                    & (
                        ...
                        | (
                            pl.col("_aux_da_final").str.strip_chars() == ""
                        )  # Vigente até hoje (vazio ou espaço)
        ...
        if col_sefin_adicional and col_sefin_adicional in df.columns:
            rprint(
                f"[blue]Buscando atributos tributários para {col_sefin_adicional}...[/blue]"
            )
            df_joined_add = (
