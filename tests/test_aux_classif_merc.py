    ...
    df_input = pl.DataFrame(
        {
            ...
            "dhsaient": [
                date(2023, 1, 1),
                date(2023, 7, 1),
            ],  # dhsaient < dhemi, so max is dhemi
        }
    )

    df_result = aux_classif_merc(
        df_input,
        col_sefin_adicional="sefin_fronteira",
        col_dhemi="dhemi",
        col_dhsaient="dhsaient",
    )
...
def test_aux_classif_merc_missing_files(mocker):
    ...
    with pytest.raises(
        FileNotFoundError,
        match="Um ou mais arquivos de referência sitafe não foram encontrados",
    ):
