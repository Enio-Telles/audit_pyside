...
def test_estoque_inicial_final_qualquer_data():
    ...
    print(
        df_mov.select(["Tipo_operacao", "Dt_doc", "q_conv", "__qtd_decl_final_audit__"])
    )
    ...
    ), f"Estoque inicial deve ser 100.0, veio {estoque_inicial}"
    ...
    assert (
        estoque_final == 100.0
    ), f"Estoque final deve ser 100.0 (50+30+20), veio {estoque_final}"

    # saidas_calculadas = estoque_inicial + entradas + entradas_desacob - estoque_final
    # saidas_calculadas = 100 + 50 + 0 - 100 = 50
    assert (
        saidas_calculadas == 50.0
    ), f"Saídas calculadas devem ser 50.0, veio {saidas_calculadas}"
...
def test_estoque_inicial_fora_01_01():
    ...
    assert (
        estoque_inicial == 250.0
    ), f"Estoque inicial em 15/06 deve ser capturado, veio {estoque_inicial}"
...
def test_estoque_final_fora_31_12():
    ...
    assert (
        estoque_final == 180.0
    ), f"Estoque final em 30/06 deve ser capturado, veio {estoque_final}"
