...
def test_calcular_fatores_conversao_remapeia_override_manual_por_descricao(
    tmp_path: Path,
):
    cnpj = "12345678000190"
    ...
    pl.DataFrame(
        {
            ...
            "lista_descricoes": [
                ["DENVER STEAK BOV ANGUS KG"],
                ["WHISKY JW BLACK 12/750ML"],
            ],
        }
    ...
    assert df_whisky.get_column("descr_padrao").unique().to_list() == [
        "WHISKY JW BLACK 12/750ML"
    ]
    assert (
        df_whisky.filter(pl.col("unid") == "CX").row(0, named=True)["fator_manual"]
        is True
    )
    assert df_whisky.filter(pl.col("unid") == "CX").row(0, named=True)["fator"] == 12.0
    assert df_whisky.get_column("unid_ref").unique().to_list() == ["UN"]

    df_log = pl.read_parquet(
        pasta_prod / f"log_reconciliacao_overrides_fatores_{cnpj}.parquet"
    )
