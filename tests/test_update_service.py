...
def mock_frozen_windows():
    with patch("sys.platform", "win32"), patch("sys.frozen", True, create=True):
        yield
...
def test_update_worker_non_windows():
...
def test_update_worker_non_frozen():
    with patch("sys.platform", "win32"), patch("sys.frozen", False, create=True):
        worker = UpdateWorker()
        with patch.object(worker, "finished") as mock_finished:
            worker.run()
            mock_finished.emit.assert_called_once_with(None)
...
def test_update_worker_no_update(mock_loop, mock_manager_class, mock_frozen_windows):
...
@patch("interface_grafica.services.update_service.QNetworkAccessManager")
@patch("interface_grafica.services.update_service.QEventLoop")
def test_update_worker_missing_checksums(mock_loop, mock_manager_class, mock_frozen_windows):
    mock_manager = mock_manager_class.return_value
    mock_reply = MagicMock(spec=QNetworkReply)
    mock_reply.error.return_value = QNetworkReply.NoError
...
@patch("interface_grafica.services.update_service.QNetworkAccessManager")
@patch("interface_grafica.services.update_service.QEventLoop")
def test_update_worker_with_update(mock_loop, mock_manager_class, mock_frozen_windows):
    mock_manager = mock_manager_class.return_value
    mock_reply = MagicMock(spec=QNetworkReply)
    mock_reply.error.return_value = QNetworkReply.NoError
    mock_reply.readAll().data().decode.return_value = '''
...
@patch("interface_grafica.services.update_service.QNetworkAccessManager")
def test_checksum_download_failure(mock_manager_class):
    mock_manager = mock_manager_class.return_value
    mock_reply = MagicMock(spec=QNetworkReply)
    mock_reply.error.return_value = QNetworkReply.ProtocolUnknownError
    mock_reply.errorString.return_value = "Unknown error"
    mock_manager.get.return_value = mock_reply
    ...
    with patch.object(service, "update_error") as mock_error:
        # Capture callback
        callback = None
        def mock_connect(cb):
            nonlocal callback
            callback = cb
        mock_reply.finished.connect.side_effect = mock_connect

        service.start_update_download(release_info)
        callback()

        mock_error.emit.assert_called_once()
        assert "Erro ao baixar checksums" in mock_error.emit.call_args[0][0]
