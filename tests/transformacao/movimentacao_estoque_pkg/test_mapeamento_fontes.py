...
def test_parse_expression_complex_cod_barra():
    df = pl.DataFrame(
        {"prod_ceantrib": ["123", None, None], "prod_cean": ["456", "789", None]}
    )
...
def test_parse_expression_math_vl_item():
    df = pl.DataFrame({"vl_item": [100.0, 50.0, 10.0], "vl_desc": [10.0, None, 2.5]})
...
def test_parse_expression_math_prod_vprod():
    ...
    expr = parse_expression(
        "prod_vprod+prod_vfrete+prod_vseg+prod_voutro-prod_vdesc", "vl_total"
    )
    res = evaluate_expr(df, expr)
    # 100 + 10 + 5 + 2 - 7 = 110.0
    # 50 + 0 + 0 + 0 - 10 = 40.0
...
def test_parse_expression_chave_nfe():
    df = pl.DataFrame({"chv_nfe": ["35210112345678901234550010000000011000000018"]})
...
def test_parse_expression_gerado_registro():
    df = pl.DataFrame({"dummy": [1]})
    expr = parse_expression(
        '"gerado" ou "registro" (se está no bloco_h)', "ind_registro"
    )
    res = evaluate_expr(df, expr)
